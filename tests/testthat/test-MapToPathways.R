...
test_that("MapToPathway correctly aggregates scores using row names and internal data", {
  ...
  # There are 5 unique pathways in metab_to_pwys. All 5 are affected in all 3 samples.
  ...
  expect_equal(glycolysis_results$Net_Score[glycolysis_results$Sample_ID == "Sample2"], 0)
  expect_equal(glycolysis_results$Activity_Status[glycolysis_results$Sample_ID == "Sample2"], "Normal Activity")
  ...
  expect_equal(bcaa_results$Metabolites_Affected[bcaa_results$Sample_ID == "Sample1"], 3) # All 3 are mapped and scored
...
# Clean up the globally assigned object after the test runs to ensure the environment is left clean.
rm(metab_to_pwys, envir = .GlobalEnv)
