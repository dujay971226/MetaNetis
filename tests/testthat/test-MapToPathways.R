...
# -----------------Test Block----------------------
test_that(
  "MapToPathway correctly aggregates scores using row names and internal data", {
  ...
  # There are 5 unique pathways in metab_to_pwys.
  # All 5 are affected in all 3 samples.
  expect_equal(nrow(result), 5 * 3)
  expect_true(all(c("Sample_ID",
                    "Pathway_Name",
                    "Net_Score",
                    "Metabolites_Affected",
                    "Activity_Status") %in% names(result)))
  ...
  # Sample 1: Low (-1) + Low (-1) = -2 (Hypoactive)
  expect_equal(glycolysis_results$Net_Score[
    glycolysis_results$Sample_ID == "Sample1"], -2)
  expect_equal(glycolysis_results$Activity_Status[
    glycolysis_results$Sample_ID == "Sample1"], "Hypoactive")

  # Sample 2: High (+1) + Low (-1) = 0 (Normal Activity)
  expect_equal(glycolysis_results$Net_Score[
    glycolysis_results$Sample_ID == "Sample2"], 0)
  expect_equal(glycolysis_results$Activity_Status[
    glycolysis_results$Sample_ID == "Sample2"], "Normal Activity")

  # Sample 3: Normal (0) + Low (-1) = -1 (Mild Inhibition)
  expect_equal(glycolysis_results$Net_Score[
    glycolysis_results$Sample_ID == "Sample3"], -1)
  expect_equal(glycolysis_results$Activity_Status[
    glycolysis_results$Sample_ID == "Sample3"], "Mild Inhibition")
  ...
  # Sample 1: High (+1) + Normal (0) + High (+1) = +2 (Hyperactive)
  expect_equal(bcaa_results$Net_Score[
    bcaa_results$Sample_ID == "Sample1"], 2)
  expect_equal(bcaa_results$Activity_Status[
    bcaa_results$Sample_ID == "Sample1"], "Hyperactive")
  expect_equal(bcaa_results$Metabolites_Affected[
    bcaa_results$Sample_ID == "Sample1"], 3) # All 3 are mapped and scored
  ...
  # Sample 3: High (+1) + High (+1) = +2 (Hyperactive)
  expect_equal(serotonin_results$Net_Score[
    serotonin_results$Sample_ID == "Sample3"], 2)
  expect_equal(serotonin_results$Activity_Status[
    serotonin_results$Sample_ID == "Sample3"], "Hyperactive")
})
